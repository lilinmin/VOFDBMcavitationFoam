...
void Foam::phaseCoupling::checkConnection(const label startCell, const label currentID)
{
    ...
    while (cells.size() > 0)
    {
        ...
        forAll(cells, i)
        {
            ...
            if (alpha1_[cellI] > alphaLimit_ && mag(vofID_[cellI] - currentID) > 0.1)
            {
                // Set volume ID
...
:
    ...
    (
        ...
        mesh_,
        dimensionedScalar("maskalpha1", dimless, scalar(0.0)),
        zeroGradientFvPatchScalarField::typeName
    )
{}
...
void Foam::phaseCoupling::update()
{
    ...
    forAll(mesh_.cells(), cellI)
    {
        ...
        if (alpha1_[cellI] > alphaLimit_ && mag(vofID_[cellI]) < 0.1)
        {
    ...
    // Calculate bubble volume, velocity, and position
    ...
    forAll(mesh_.cells(), cellI)
    {
        if (vofID_[cellI] > 0.1)
        {
            ...
            volume[volID] += alpha1_[cellI]*mesh_.V()[cellI];
            velocity[volID] += alpha1_[cellI]*mesh_.V()[cellI]*U_[cellI];
            position[volID] += alpha1_[cellI]*mesh_.V()[cellI]*mesh_.C()[cellI];
        }
    }
    ...
    for(int i=1; i<=maxID; i++)
    {
        if (volume[i] > VSMALL)
        {
            ...
            // bubble must be smaller than dMax_
            if ( d < dMax_ ) 
            {
                // Average cell size for bubble
                scalar dx = Foam::cbrt(cellVolume[i] / noCells[i]);

                // bubble diameter has to larger than 0.5 average cell size
                // to suppress very small bubbles
                if (d > 0.5*dx)
                {
                    ...
                    if ( sph < sphMax_ )
                    {
                        ...
                        // Inject bubble
                        cloud_.inject(pos, d, U, cloud_);

                        // Set alpha field and adjust velocity damping field
                        damping_ += GREAT*mask/deltaT;
                        maskalpha1_ = mask*alpha1_;
                        alpha1_ = max(0.0,alpha1_ - 1000.0*mask*alpha1_);
                        alpha2_ = min(1.0,alpha2_ + 1000.0*mask*alpha1_);
                    }
