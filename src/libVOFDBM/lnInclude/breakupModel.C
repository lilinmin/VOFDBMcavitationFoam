...
)
{
    if(model_ == "ETAB")
    {
        ...
        {
            // reset bubble distortion parameters
            y = 0;
            yDot = 0;
        }
    ...
    else if (model_ == "ReitzDiwakar")
    {
        ...
        if (We > Cbag_)
        {
            if (We > Cstrip_*sqrt(Re))
            {
                ...
                // new bubble diameter, implicit calculation
    ...
    else if (model_ == "PilchErdman")
    {
        ...
        if (We > Wec)
        {
            ...
            if (d < dStable)
            {
                // bubble diameter already stable = no break-up
            ...
            else
            {
                ...
                // update bubble diameter according to the rate eq (implicitly)
                scalar frac = dt/taub;
