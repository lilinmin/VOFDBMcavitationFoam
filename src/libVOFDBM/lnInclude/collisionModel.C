...
bool Foam::collisionModel::collidebubbles
(
    const scalar dt,
    bubble& p1,
    bubble& p2,
...
)
{
    ...
    // bubbles travel towards each other
    if (vAlign > 0)
    {
...
)
{
    ...
    // Store cloud bubble properties
    rhop_ = cloud.rhop();
    sigma_ = cloud.sigma();

    // Create pointer to cloud
    bubbleCloud *ptrCloud = const_cast<bubbleCloud*>(&cloud);    

    // Create the occupancy list for the cells
    labelList occupancy(mesh_.nCells(), 0);
    forAllIter(Cloud<bubble>, *ptrCloud,  iter)
    {
        occupancy[iter().cell()]++;
    }

    // Initialize the sizes of the lists of parcels in each cell
    CompactListList<bubble*> pInCell(occupancy);
    ...
    // Perform bubble-bubble collision per cell
    for (label celli=0; celli < mesh_.nCells(); celli++)
    {
        UList<bubble*> pInCelli(pInCell[celli]);
        ...
        if (pInCelli.size() >= 2)
        {
            forAll(pInCelli, i)
            {
                ...
                {
                    bubble& p1 = *pInCelli[i];
                    bubble& p2 = *pInCelli[j];
                    ...
                    bool massChanged = collidebubbles(dt, p1, p2, m1, m2);
    ...
    // Delete all bubbles with negative diameter
    forAllIter(Cloud<bubble>, *ptrCloud, iter)
    {
        bubble& p = iter();
